...
def smoothness_loss(depth_map, image):
    # Nếu depth_map có thêm chiều batch, ta loại bỏ nó
    if depth_map.dim() == 3:  # (1, H, W)
        depth_map = depth_map.squeeze(0)  # (H, W)

    grad_depth_x = torch.abs(depth_map[:, :-1] - depth_map[:, 1:])  # Gradient theo chiều ngang
    grad_depth_y = torch.abs(depth_map[:-1, :] - depth_map[1:, :])  # Gradient theo chiều dọc

    # Tính gradient của ảnh RGB (hoặc ảnh xám)
    grad_img_x = torch.mean(torch.abs(image[:, :, :-1] - image[:, :, 1:]), dim=0)  # Trung bình trên kênh màu
    grad_img_y = torch.mean(torch.abs(image[:, :-1, :] - image[:, 1:, :]), dim=0)

    # Trọng số dựa trên độ mượt của ảnh
    weight_x = torch.exp(-grad_img_x)
    weight_y = torch.exp(-grad_img_y)

    # Loss mượt mà
    smooth_loss_x = (grad_depth_x * weight_x).mean()
    smooth_loss_y = (grad_depth_y * weight_y).mean()
...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        unskip_np = viewpoint_cam.processed_mask
        # print(f"\n Type {unskip_np}\n")
        skip_np = 1 - unskip_np
        unskip_mask = torch.tensor(unskip_np).cuda()
        skip_mask = torch.tensor(skip_np).cuda()
        midas_depth = torch.tensor(viewpoint_cam.depth_image).cuda()

        unskip_mask = unskip_mask.reshape(-1, 1)
        skip_mask = skip_mask.reshape(-1, 1)
        rendered_depth_unskip = rendered_depth.reshape(-1, 1) * unskip_mask
        rendered_depth_skip = rendered_depth.reshape(-1, 1) * skip_mask 
        midas_depth_unskip = midas_depth.reshape(-1, 1) * unskip_mask
